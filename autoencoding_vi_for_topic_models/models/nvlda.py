...
class VAE(object):
    ...
    def _create_network(self):
        ...
        if USE_NGS:
            ...
            self.z = tf.add(self.z_mean,
                            tf.multiply(tf.sqrt(tf.exp(self.z_log_sigma_sq)), self.eps))
    ...
    def partial_fit(self, X):
        opt, cost,emb = self.sess.run((self.optimizer, self.cost,self.network_weights['weights_gener']['h2']),feed_dict={self.x: X,self.keep_prob: .75, self.eps: np.random.random([X.shape[0], self.network_architecture["n_z"]])})
        return cost,emb

    def test(self, X):
        cost = self.sess.run((self.cost),
                                        feed_dict={self.x: np.expand_dims(X, axis=0),self.keep_prob: 1.0, self.eps: np.random.random([1, self.network_architecture["n_z"]])})
        return cost
